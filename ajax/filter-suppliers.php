...
if (!empty($search)) {
    $where_conditions[] = "(s.name LIKE ? OR s.contact_person LIKE ? OR s.phone LIKE ? OR s.email LIKE ? OR s.address LIKE ?)";
    $params[] = "%$search%";
    $params[] = "%$search%";
    $params[] = "%$search%";
    $params[] = "%$search%";
...
switch ($sort) {
    case 'products_count':
        $order_clause .= 'products_count DESC, s.name ASC';
        break;
    case 'created_at':
        $order_clause .= 's.created_at DESC, s.name ASC';
        break;
    default:
...
if (mysqli_num_rows($result) == 0) {
    ...
    $output .= '<i class="fas fa-search text-4xl text-gray-300 mb-4"></i>';
    $output .= '<div class="text-gray-500 text-lg font-medium">No suppliers found</div>';
    $output .= '<div class="text-gray-400 text-sm">Try adjusting your search criteria</div>';
    $output .= '</div>';
    $output .= '</td></tr>';
    ...
    $mobile_output .= '<i class="fas fa-search text-3xl text-gray-300 mb-3"></i>';
    $mobile_output .= '<div class="text-gray-500 text-base font-medium">No suppliers found</div>';
    $mobile_output .= '<div class="text-gray-400 text-xs">Try adjusting your search criteria</div>';
    $mobile_output .= '</div>';
    $mobile_output .= '</div>';
