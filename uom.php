    ...
    <div class="p-4 sm:p-6 lg:p-8">
        ...
        <div class="backdrop-blur-sm bg-white/70 rounded-2xl shadow-xl border border-white/20 p-6 mb-8">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-4 sm:space-y-0">
                <div>
                    ...
                    <p class="text-gray-600 text-lg">Manage your product units of measure efficiently</p>
                </div>
                <button onclick="openAddModal()" class="group relative inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
                    ...
                    <span class="relative z-10 font-medium">Add UOM</span>
                </button>
            </div>
        </div>
        ...
        <div class="backdrop-blur-sm bg-white/70 rounded-xl lg:rounded-2xl shadow-xl border border-white/20 p-4 lg:p-6 mb-6 lg:mb-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
                <div class="space-y-2">
                    <label class="block text-xs lg:text-sm font-semibold text-gray-700">Search UOM</label>
                    <div class="relative group">
                        ...
                        <input type="text" id="search-input" placeholder="Search UOM..." 
        ...
        <div class="backdrop-blur-sm bg-white/70 rounded-xl lg:rounded-2xl shadow-xl border border-white/20 overflow-hidden">
            <div class="px-4 lg:px-6 py-4 lg:py-6 border-b border-gray-200/50 bg-gradient-to-r from-gray-50/50 to-gray-100/50">
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-2 lg:space-x-3">
                        ...
                        <h3 class="text-lg lg:text-xl font-semibold text-gray-900">Units of Measure List</h3>
                    </div>
                    <div class="flex items-center space-x-1 lg:space-x-2 text-xs lg:text-sm text-gray-600">
                        <i class="fas fa-info-circle"></i>
                        <span id="uom-count">0 UOMs</span>
                    </div>
                </div>
            </div>
            ...
            <div class="block lg:hidden">
                <div id="uom-mobile" class="divide-y divide-gray-200/50">
            ...
            <div class="hidden lg:block overflow-x-auto">
                <table id="uom-table" class="w-full">
                    <thead class="bg-gradient-to-r from-gray-50/80 to-gray-100/80">
                        <tr>
                            <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">UOM Name</th>
                            <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Abbreviation</th>
                    ...
                    <tbody id="uom-tbody" class="bg-white/50 divide-y divide-gray-200/50">
                        <!-- UOM will be loaded here -->
                    </tbody>
                </table>
            </div>
                ...
                <div class="max-w-md mx-auto">
                    <div class="relative mb-6">
                        <div class="w-24 h-24 mx-auto bg-gradient-to-br from-gray-100 to-gray-200 rounded-full flex items-center justify-center">
                            <i class="fas fa-ruler text-3xl text-gray-400"></i>
                        </div>
                    ...
                    <h3 class="text-xl font-semibold text-gray-900 mb-2">No UOMs found</h3>
                    <p class="text-gray-500 mb-6">Get started by adding your first unit of measure to the system.</p>
                    <button onclick="openAddModal()" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
                        <i class="fas fa-plus mr-2"></i>Add Your First UOM
                    </button>
                </div>
            </div>
        </div>
...
<!-- Add/Edit UOM Modal with Glassmorphism -->
<div id="uom-modal" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden z-50">
    <div class="flex items-center justify-center min-h-screen p-6">
        <div class="backdrop-blur-md bg-white/95 rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-white/20 m-6">
            <div class="p-8 border-b border-gray-200/50 bg-gradient-to-r from-gray-50/50 to-gray-100/50">
                <div class="flex justify-between items-center">
                    <div class="flex items-center space-x-3">
                        ...
                        <h3 class="text-2xl font-bold text-gray-900" id="modal-title">Add New UOM</h3>
                    </div>
            ...
            <div class="p-8">
                <form id="uom-form" class="space-y-6">
                    <input type="hidden" id="uom-id" name="id">
                    ...
                    <div class="space-y-6">
                        <div class="flex items-center space-x-3 mb-6">
                            ...
                            <h4 class="text-lg font-semibold text-gray-900">UOM Information</h4>
                        </div>
                        
                        <div class="space-y-2">
                            <label class="block text-sm font-semibold text-gray-700">UOM Name *</label>
                        ...
                        <div class="space-y-2">
                            <label class="block text-sm font-semibold text-gray-700">Abbreviation</label>
                        ...
                        <div class="flex items-center space-x-3">
                            ...
                            <label for="is_active" class="text-sm font-medium text-gray-700">Active UOM</label>
                        </div>
                    </div>
                </form>
            </div>
...
$(document).ready(function() {
    ...
    $('#uom-table').DataTable({
        ...
        language: {
            ...
            lengthMenu: "Show _MENU_ UOMs per page",
            info: "Showing _START_ to _END_ of _TOTAL_ UOMs",
            paginate: {
...
function loadUOM() {
    ...
    $.ajax({
        url: 'ajax/get-uom-list.php',
        type: 'GET',
        success: function(response) {
            $('#uom-tbody').html(response);
            
            // Update UOM count
            const uomCount = $('#uom-tbody tr').not('.no-data').length;
            $('#uom-count').text(uomCount + ' UOMs');
            
            // Check if table is empty
            if ($('#uom-tbody tr').length === 0 || $('#uom-tbody tr').hasClass('no-data')) {
        ...
        error: function() {
            showAlert('Error loading UOMs', 'error');
        }
    });
}
...
function updateMobileView() {
    const mobileContainer = $('#uom-mobile');
    ...
    if (mobileData.length > 0) {
        ...
        mobileContainer.html('<div class="p-8 text-center text-gray-500">No UOMs found</div>');
    }
}
...
function openEditModal(id) {
    ...
    const modalContent = $('#uom-modal .backdrop-blur-md');
    modalContent.append('<div id="loading-overlay" class="absolute inset-0 bg-white/80 backdrop-blur-sm flex items-center justify-center z-10"><div class="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div></div>');
    
    $.ajax({
        url: 'ajax/get-uom-data.php',
        type: 'GET',
        data: { id: id },
        success: function(response) {
            try {
                const uom = JSON.parse(response);
                
                if (uom.error) {
                    showAlert(uom.error, 'error');
                    closeModal();
                    return;
                }
                ...
                $('#uom-id').val(uom.id);
                $('#name').val(uom.name);
                $('#abbreviation').val(uom.abbreviation);
                $('#description').val(uom.description);
                $('#is_active').prop('checked', uom.is_active == 1);
                
            } catch (e) {
                showAlert('Error parsing UOM data', 'error');
                closeModal();
            }
        },
        error: function(xhr, status, error) {
            showAlert('Error loading UOM data: ' + error, 'error');
            closeModal();
        },
        complete: function() {
...
function saveUOM() {
    ...
    if (!name) {
        showAlert('UOM name is required', 'error');
        $('#name').focus();
        return;
    }
    
    const formData = new FormData($('#uom-form')[0]);
    const uomId = $('#uom-id').val();
    
    // Determine if this is an edit or add operation
    const url = uomId ? 'ajax/edit-uom.php' : 'ajax/save-uom.php';
    
    // Show loading state
    const saveButton = $('button[onclick="saveUOM()"]');
    ...
    $.ajax({
        ...
        error: function(xhr, status, error) {
            showAlert('Error saving UOM: ' + error, 'error');
        },
        complete: function() {
            // Reset button state
...
function deleteUOM(id) {
    ...
    const confirmDialog = $(`
        <div class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div class="backdrop-blur-md bg-white/95 rounded-2xl shadow-2xl max-w-md w-full border border-white/20">
                ...
                <div class="p-6">
                    <p class="text-gray-600 mb-4">Are you sure you want to delete this UOM? This action cannot be undone.</p>
                    <div class="flex justify-end space-x-3">
                        <button onclick="closeDeleteDialog()" class="px-4 py-2 text-gray-600 border border-gray-300 rounded-lg hover:bg-gray-50 transition-all duration-200">
                            Cancel
                        </button>
                        <button onclick="confirmDeleteUOM(${id})" class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-all duration-200">
                            Delete
                        </button>
                    </div>
                </div>
...
function closeDeleteDialog() {
    $('.fixed.inset-0.bg-black\\/50').remove();
}

function confirmDeleteUOM(id) {
    closeDeleteDialog();
    
    // Show loading state
    showAlert('Deleting UOM...', 'info');
    
    $.ajax({
        url: 'ajax/delete-uom.php',
        type: 'GET',
        data: { id: id },
        success: function(response) {
            try {
                ...
                showAlert('Error deleting UOM', 'error');
            }
        },
        error: function() {
            showAlert('Error deleting UOM', 'error');
...
$(document).on('click', '#uom-modal', function(e) {
    if (e.target === this) {
        closeModal();
    }
});
...
$(document).on('keydown', function(e) {
    if (e.key === 'Escape' && !$('#uom-modal').hasClass('hidden')) {
        closeModal();
    }
});
</script>
