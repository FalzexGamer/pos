...
<!-- Custom POS Sidebar -->
<aside class="sidebar fixed left-0 top-0 h-full w-64 bg-white shadow-lg transform transition-transform duration-300 ease-in-out z-40 lg:translate-x-0" style="top: 64px;">
    <div class="p-4">
        ...
        <!-- Categories and Products Dropdown -->
        <div class="space-y-4 sidebar-content">
            ...
            <div class="space-y-2">
                <button class="w-full flex items-center justify-between px-4 py-3 text-gray-700 rounded-lg hover:bg-blue-50 hover:text-blue-600 transition-colors border border-gray-200" 
                        onclick="toggleCategory('category-<?= $category['id'] ?>')">
                    <div class="flex items-center">
                        <i class="fas fa-folder w-5 h-5 mr-3 text-blue-500"></i>
                        <span class="font-medium"><?= htmlspecialchars($category['name']) ?></span>
                    </div>
                    <i class="fas fa-chevron-down text-xs transition-transform" id="arrow-<?= $category['id'] ?>"></i>
                </button>
                
                <div id="category-<?= $category['id'] ?>" class="hidden pl-8 space-y-2">
                    <?php if (empty($category['products'])): ?>
                        <div class="text-sm text-gray-500 italic px-4 py-2">No products in this category</div>
                    <?php else: ?>
...
function toggleCategory(categoryId) {
    ...
    const arrow = document.getElementById(categoryId.replace('category-', 'arrow-'));
    
    category.classList.toggle('hidden');
    arrow.style.transform = category.classList.contains('hidden') ? 'rotate(0deg)' : 'rotate(180deg)';
}
