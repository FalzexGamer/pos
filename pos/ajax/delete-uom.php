...
$check_sql = "SELECT COUNT(*) as count FROM products WHERE uom_id = ?";
...
if ($result['count'] > 0) {
    echo json_encode(['success' => false, 'message' => 'Cannot delete UOM. It is being used by ' . $result['count'] . ' product(s)']);
    exit;
}

// Delete the UOM
$sql = "DELETE FROM uom WHERE id = ?";
